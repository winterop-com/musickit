...
(function () {
  ...
  // The web UI server-renders #audio + #viz-canvas, so they exist by
  // the time this script's deferred run happens. The desktop SPA boots
  // through an async login, so the same elements may not exist until
  // shell.js renders them later. Wrap the whole init in a function and
  // retry via MutationObserver until both elements are present.
  ...
  function initVisualizer(audio, canvas) {
    // The fullscreen-toggle button is optional — the web UI ships one,
    // the desktop SPA doesn't.
    ...
    // Click-to-fullscreen only available when the host page provides a
    // toggle button (the web UI does; the desktop SPA doesn't yet).
